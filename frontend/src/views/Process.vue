      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ t('process.dataShowingSoon') }}</p>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ t('process.waitingOntologyHint') }}</div>
              </div>
            </div>
          </div>
...
const goToNextStep = () => {
  // TODO: {{ t('process.enterEnvSetup') }}步骤
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = t('process.projectInitFailed') + ': ' + (err.message || t('process.unknown'))
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    error.value = t('process.loadProjectFailed') + ': ' + (err.message || t('process.unknown'))
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    error.value = t('process.graphBuildStartFailed') + ': ' + (err.message || t('process.unknown'))
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.graphBuildFailed') + ': ' + (task.error || t('process.unknown'))
        buildProgress.value = null
      }
    }
  } catch (err) {
